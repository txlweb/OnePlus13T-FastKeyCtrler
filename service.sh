...
pgrep -f "$MODDIR/service.sh" | while read pid; do
    if [ "$pid" != "$$" ]; then
        kill "$pid"
    fi
done
...
echo "开始监听 $KEY..."
sed -i '/^description=/d' "$MODDIR/module.prop"
echo "description=[?] 服务已启动，按下按键来测试是否生效。" >> $MODDIR/module.prop
...
while true; do
    getevent -lt "$DEVICE" | while read -r line; do
        ...
        set -- $line
        sed -i '/^description=/d' "$MODDIR/module.prop"
        echo "description=[√] 按键功能已经生效" >> $MODDIR/module.prop
...
sed -i '/^description=/d' "$MODDIR/module.prop"
echo "description=[x] 服务被杀死，请尝试手动重启..." >> $MODDIR/module.prop